...
export const submitSequence = async (
  ...
  try {
    ...
    const jobId = uuidv4();
    const blobName = `proteins/${jobId}.pdb`;

    const storageUrl = await generateSasUrl(blobName);
  ...
  try {
    ...
    if (loggedJob) {
      ...
      if (
        ...
        const storageUrl = await generateSasUrl(loggedJob.blobName);
        const blobExists = await checkBlobExists(loggedJob.blobName);
...
export const checkResult = async (
  ...
  try {
    ...
    try {
      const exists = await checkBlobExists(loggedJob.blobName);
